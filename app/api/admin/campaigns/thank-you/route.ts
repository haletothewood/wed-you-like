...
import { getCampaignBaseUrl, withCampaignLock } from '../_shared'
...
export async function POST(request: Request) {
  try {
    const result = await withCampaignLock(
      'campaign:thank-you',
      async (): Promise<ThankYouCampaignResponse> => {
        ...
        const campaignResult: ThankYouCampaignResponse = {
          success: true,
        ...
        for (const recipient of plan.recipients) {
          ...
          try {
            ...
            campaignResult.errors.push(`Failed for ${recipient.email}: ${message}`)
          }
        }
